...
multicuts_descriptor = {
    ...
    # "initial_mass": 800,
    # "num_stages": 3,
    # "solid_zone": [lambda x: np.full(x.shape[1], False), lambda x: np.greater(x[2], 9.6)],
    # "void_zone": [lambda x: np.greater(x[2], 9.6), lambda x: np.full(x.shape[1], False)],
    # "solid_zone": [lambda x: np.full(x.shape[1], False),  lambda x: np.full(x.shape[1], False), lambda x: np.greater(x[2], 9.6)],
    # "void_zone": [lambda x: np.greater(x[2], 9.6),  lambda x: np.greater(x[2], 9.6), lambda x: np.full(x.shape[1], False)],
    "solid_zone": [lambda x: np.full(x.shape[1], False), lambda x: np.full(x.shape[1], False), lambda x: np.full(x.shape[1], False), lambda x: np.greater(x[2], 9.6)],
    "void_zone": [lambda x: np.greater(x[2], 9.6), lambda x: np.greater(x[2], 9.6), lambda x: np.greater(x[2], 9.6), lambda x: np.full(x.shape[1], False)],
    "num_divisions": 100,
